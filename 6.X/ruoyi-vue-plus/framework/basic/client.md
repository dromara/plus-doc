...
通常对应前端环境变量 `VITE_APP_CLIENT_ID`，本地开发一般修改 `.env.development`，生产环境按实际部署环境修改对应 `.env` 文件。
...
补充说明：
- `plus-ui` 的普通接口请求、SSE、WebSocket 等能力都会读取这个客户端 ID
- 如果你只改了某个单独页面而没有统一修改环境变量，联调时很容易出现“客户端ID与Token不匹配”
