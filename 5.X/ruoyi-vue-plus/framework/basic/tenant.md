...
在使用多租户前，请先明确系统定位。多租户适用于**一套系统服务多家独立企业**的场景，租户之间数据必须完全隔离。

常见误区：
- 平台内的多商户、本部/分部、大租户/小租户、上下级租户等，**不是**多租户问题
- 这类场景通常应使用**数据权限**与业务逻辑来解决

![输入图片说明](https://foruda.gitee.com/images/1747208232753381691/bb2ab775_1766278.png "屏幕截图")

总结：SaaS 本质是“卖软件的”，客户之间应互不关联。请确认适用后再启用。
...
> 开关 `enable`：控制多租户总开关。<br>
> `excludes`：不需要租户隔离的表可在此配置排除。
...
>
> 补充说明：
> 1、这里的“同步套餐”本质上是把当前套餐关联的菜单权限重新同步到目标租户。
> 2、主要影响的是租户管理员及其角色可分配的菜单范围，不是重置租户业务数据。
> 3、如果你后续还调整了默认租户下的字典、参数等基础配置，通常还需要配合同步字典/参数这类能力一起处理。
